...
const STORES = [
  {
    ...
    isActive: true,
    hasCoordinates: true,
    assignedRoute: { id: 3, code: 'RT-BT-01', name: 'Tuyến Bình Thạnh 01' },
...
function fillRequiredStoreFields(code: string, name: string, address: string) {
  ...
  cy.get('textarea[placeholder="VD: 120 Nguyễn Oanh, P.17, Q.Gò Vấp"]').type(address);
}
...
describe('US-04 — Store Management', () => {
  ...
  it('TC-02: tạo store mới thành công, payload được trim và uppercase code', () => {
    ...
    cy.intercept('POST', '**/api/stores', (req) => {
      expect(req.body).to.deep.include({
        ...
        address: '18 Phan Văn Trị, Gò Vấp',
    ...
    cy.get('input[placeholder="VD: ST-GV-005"]').type('st-gv-006');
    cy.get('input[placeholder="VD: Điện Máy Phúc Anh"]').type('Điện Máy Gia Phát');
    cy.get('textarea[placeholder="VD: 120 Nguyễn Oanh, P.17, Q.Gò Vấp"]').type(
  ...
  it('TC-03: nhập thiếu một trong hai toạ độ thì validate inline', () => {
    ...
    cy.contains('button', 'Thêm cửa hàng').click();
    cy.get('input[placeholder="VD: ST-GV-005"]').type('ST-GV-007');
    cy.get('input[placeholder="VD: Điện Máy Phúc Anh"]').type('Store thiếu kinh độ');
    cy.get('textarea[placeholder="VD: 120 Nguyễn Oanh, P.17, Q.Gò Vấp"]').type(
      '88 Nguyễn Kiệm, Gò Vấp'
    );
  ...
  it('TC-04: duplicate storeCode từ backend được map về field Mã cửa hàng', () => {
    ...
    cy.contains('button', 'Thêm cửa hàng').click();
    cy.get('input[placeholder="VD: ST-GV-005"]').type('ST-BT-001');
    cy.get('input[placeholder="VD: Điện Máy Phúc Anh"]').type('Store trùng mã');
    cy.get('textarea[placeholder="VD: 120 Nguyễn Oanh, P.17, Q.Gò Vấp"]').type(
      '99 Nguyễn Oanh, Gò Vấp'
    );
    ...
    cy.contains('.ant-form-item-explain-error', 'Mã cửa hàng đã tồn tại.').should('be.visible');
  });
  ...
  it('TC-07: tạo store thiếu field bắt buộc thì hiển thị validation errors', () => {
    ...
    cy.contains('.ant-form-item-explain-error', 'Vui lòng nhập mã cửa hàng.').should('be.visible');
    cy.contains('.ant-form-item-explain-error', 'Vui lòng nhập tên cửa hàng.').should('be.visible');
    cy.contains('.ant-form-item-explain-error', 'Vui lòng nhập địa chỉ.').should('be.visible');
  ...
  it('TC-10: tạo store không có GPS vẫn thành công và hiện cảnh báo inline', () => {
    ...
    cy.wait('@getStores');
    cy.contains('button', 'Thêm cửa hàng').click();
    cy.contains('.ant-alert', 'Chưa có toạ độ GPS.').should('be.visible');
  ...
  it('TC-14: form chỉnh sửa giữ storeCode readonly và chỉ gửi field được phép sửa', () => {
    ...
    cy.get('input[placeholder="VD: ST-GV-005"]').should('have.attr', 'readonly');
    cy.get('textarea[placeholder="VD: 120 Nguyễn Oanh, P.17, Q.Gò Vấp"]')
      .clear()
