  ...
  cy.intercept(
    ...
    apiSuccess({
      ...
      totalMaxWeightKg: data
        ...
        .reduce((sum, vehicle) => sum + (vehicle.payloadKg || vehicle.maxWeightKg), 0),
...
describe('US-06 — Vehicle Management', () => {
  ...
  it('TC-02: đăng ký xe mới thành công và gửi đúng payload', () => {
    ...
    cy.get('#vehicleCode').type('XE-008');
    cy.get('#plateNumber').type('51D-88888');
  ...
  it('TC-03: biển số vô nghĩa bị chặn ở validation frontend', () => {
    ...
    cy.contains('button', 'Đăng ký xe').click();
    cy.get('#vehicleCode').type('XE-009');
    cy.get('#plateNumber').type('------');
  ...
  it('TC-04: chỉnh sửa xe không gửi plateNumber vì biển số là immutable', () => {
    ...
    cy.get('#plateNumber').should('have.attr', 'disabled');
    cy.get('#vehicleType')
      .clear()
  ...
  it('TC-07: đăng ký xe thiếu field bắt buộc thì hiển thị validation errors', () => {
    ...
    cy.contains('.ant-form-item-explain-error', 'Vui lòng nhập mã xe.').should('be.visible');
    ...
    cy.contains('.ant-form-item-explain-error', 'Vui lòng nhập tải hàng cho phép.').should('be.visible');
  ...
  it('TC-08: tải trọng bằng 0 bị chặn, thể tích bằng 0 được clamp về min trước khi gọi API', () => {
    ...
    cy.contains('button', 'Đăng ký xe').click();
    cy.get('#vehicleCode').type('XE-009');
    cy.get('#plateNumber').type('51E-99999');
  ...
  it('TC-09: duplicate plateNumber từ backend được map về field Biển số', () => {
    ...
    cy.contains('button', 'Đăng ký xe').click();
    cy.get('#vehicleCode').type('XE-009');
    cy.get('#plateNumber').type('51B-67890');
    cy.get('#vehicleType').type('Xe tải nhỏ');
    cy.get('#payloadKg').type('2500');
    cy.get('#maxVolumeM3').type('12.5');
  ...
  it('TC-10: DRIVER không có quyền đọc Vehicle Management thì bị đưa về dashboard hoặc 403', () => {
    cy.intercept('GET', '**/api/vehicles*').as('getVehiclesShouldNotRun');

    visitAs('/vehicles', ['DRIVER'], 'driver01');

    cy.url().should('match', /\/(dashboard|403)/);
  ...
  it('TC-11: Fleet Capacity Card cập nhật ngay sau khi đăng ký xe mới', () => {
    ...
    cy.get('#plateNumber').type('51D-88888');
    cy.get('#vehicleType').type('Xe tải lớn');
    cy.get('#payloadKg').type('3490.5');
    cy.get('#maxVolumeM3').type('16.25');
  ...
  it('TC-15: LOGISTICS_MANAGER chỉ có quyền xem vehicle', () => {
    interceptVehicleList();
  ...
  it('TC-16: capacity thập phân hợp lệ được gửi không mất độ chính xác', () => {
    interceptVehicleList();
    cy.intercept('POST', '**/api/vehicles', (req) => {
      expect(req.body.payloadKg).to.eq(3490);
    ...
    cy.contains('button', 'Đăng ký xe').click();
    cy.get('#vehicleCode').type('XE-009');
    cy.get('#plateNumber').type('51F-12345');
    cy.get('#vehicleType').type('Xe tải trung');
    cy.get('#payloadKg').type('3490');
    cy.get('#maxVolumeM3').type('16.250');
    cy.get('.ant-modal').within(() => cy.contains('button', 'Đăng ký xe').click());
