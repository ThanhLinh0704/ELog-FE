...
console.log('Original template xml loaded, size:', xml.length);
...
replacements.forEach(({ from, to }) => {
  ...
  xml = xml.replace(new RegExp(safeFrom, 'g'), to);
});

zip.updateFile('word/document.xml', Buffer.from(xml, 'utf8'));
zip.writeZip(outputPath);

console.log('Successfully updated Word Document:', outputPath);
